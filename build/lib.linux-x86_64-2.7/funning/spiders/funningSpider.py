...
class funningSpider(CrawlSpider):
	...
	rules = {
		...
		Rule(LinkExtractor(allow='dongtai/list_4_\d+\.html$'), callback='dongtai_parse_content'),
		Rule(LinkExtractor(allow='xiegif/list_18_\d+\.html$'), callback='xiegif_parse_content'),
		Rule(LinkExtractor(allow='quweitupian/list_2_\d+\.html$'), callback='image_parse_content'),
		Rule(LinkExtractor(allow='wenzixiaohua/list_1_\d+\.html$'), callback='text_parse_content')
	}
